...
public final class AuthenticationManagerBuilderConfig {
	...
	public void builder() throws Exception {
		this.aManagerBuilder.inMemoryAuthentication().withUser(USUARIO_PADRAO).password("{noop}" + SENHA_PADRAO)
				.roles(ROLE_USER).and().withUser(USUARIO_ADMIN).password("{noop}" + SENHA_PADRAO)
				.roles(ROLE_USER, ROLE_ADMIN, ROLE_ACTUATOR);
	}
