...
public final class HttpSecurityConfig {
	...
	public void builder() throws Exception {
		this.aHttpSecurity.cors().configurationSource(getCorsBuilder()).and().httpBasic().and().authorizeRequests()
				.antMatchers(MATCHER_ALL).hasRole(ROLE_USER).and().csrf().disable().headers().frameOptions().disable();
	}

	private CorsConfigurationSource getCorsBuilder() {

		CorsConfiguration configuration = new CorsConfiguration().applyPermitDefaultValues();
		configuration.setAllowedMethods(Arrays.asList(HttpMethod.POST.toString(), HttpMethod.DELETE.toString(),
